# 📚 Documentación de Mobile App

Esta carpeta contiene la documentación técnica de la aplicación móvil.

## 📖 Contenido

### Documentación Principal
- **[INICIO_RAPIDO_APK.md](INICIO_RAPIDO_APK.md)** - Guía rápida para generar APK
- **[GUIA_GENERAR_APK.md](GUIA_GENERAR_APK.md)** - Guía completa para generar APK de Android
- **[GUIA_EMULADOR_ANDROID.md](GUIA_EMULADOR_ANDROID.md)** - Guía para ejecutar la app en emulador Android
- **[ANALISIS_ROADMAP.md](ANALISIS_ROADMAP.md)** - Análisis del roadmap y estado de funcionalidades

### Documentación Adicional
- **[../mobile/medical-app/README.md](../../mobile/medical-app/README.md)** - Documentación principal de la app (en el componente)
- **[../mobile/README.md](../../mobile/README.md)** - Documentación general de mobile

## 🎯 Audiencia

- **Desarrolladores**: Guías de desarrollo y generación de APK
- **DevOps**: Guías de build y deployment

