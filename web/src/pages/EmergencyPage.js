...
const EmergencyPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(payload) ? payload : payload?.emergencies || [];
      setRecords(list);
      setTotal(res.data?.total || res.data?.pagination?.total || list.length);
    } catch (e) {
      setError(e.response?.data?.message || 'Error al cargar emergencias.');
    } finally {
      setLoading(false);
    }
  }, [page, token]);
            ...
            <tbody>
              {records.length === 0 ? (
                ...
                <tr key={r._id}>
                  ...
                  <td>{r.chiefComplaint || r.complaint || r.description || '—'}</td>
                  <td>
                    <span className={`status-badge status-${r.severity || 'medium'}`}>
                      {SEVERITY_LABELS[r.severity] || r.severity || 'Media'}
                    </span>
                  </td>
                  <td>
